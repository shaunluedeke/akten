...
<div id="content">
    ...
    <center>
        ...
        <div class="inter" style="width: 80%;position: absolute;top: 100px;left: 50%;transform: translate(-50%, 0%);">
            <br>
            <form action="index.php?site=fine" method="post" id="catform" style="width: 200px">
                <select class="form-control" id="fine" name="cat" style="width: 100%;"
                        onchange="document.getElementById('catform').submit();">
                    <option value="">{fine_all_cat}</option>
                    <option value="all" style='color:#000000'>Alle Kategorien</option>
                    {loop fine_catloop}
                        <option value="{fine_catloop_selected}" style='color:#000000'>{fine_catloop_name}</option>
                    {/loop fine_catloop}
                </select>
            </form>
            <br>
            <table id="Table" class="table table-striped table-dark" style="width: 100%;" data-toggle="table"
                ...
                <tr>
                    ...
                    <th scope="col" data-sortable="true" data-field="">Zusammen Rechnen</th>
                ...
                {loop fine_loop}
                    <tr>
                        ...
                        <td><input type="checkbox" onclick="addprice(this)" value="{fine_loop_fine}"></td>
                ...
                {if leader3}
                    <tr aria-sort="none">
                        <td colspan="6" style="text-align: center"><a class="btn btn-success"href="index.php?site=fine-add">Hinzufügen</a></td>
                    </tr>
                {/if leader3}
                </tbody>
            </table>
            <p style="color:white">Der Preis ist <u id="result"></u> $</p>
        </div>
    </center>
</div>
{/if hasfine}
...
<script>
    ...
    function addprice(self){
        if(self.checked){
            if(!isNaN(parseFloat(self.value))) {
                price += parseFloat(self.value);
            }
        }else{
            if(!isNaN(parseFloat(self.value))) {
                price -= parseFloat(self.value);
